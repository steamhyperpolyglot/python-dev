...
def letter_pair_filter(filter_2):
    ...
    first_pair_rejects = ['dt', 'lr', 'md', 'ml', 'mr', 'mt', 'mv',
                          'td', 'tv', 'vd', 'vl', 'vm', 'vr', 'vt']
    ...
    if 'voldermort' in filter_3:
        print('Voldermort found!', file=sys.stderr)
    return filter_3


def view_by_letter(name, filter_3):
    """Filter by anagrams startin with input letter."""
    print("Remaining letters = {}".format(name))
    first = input("selecting a starting letter or press Enter to see all: ")
    subset = []
