...
# key with spaces between numbers; negative to 
# read UP column (ex = -1 2 -3 4):
key = """ -1 2 -3 4 """
...
def validate_col_row(cipherlist):
    ...
    if ROWS * COLS != len_cipher:
        print("\nError - Input columns & rows not factors of ", 
            "length of cipher. Terminating program.", \
            file=sys.stderr)
    sys.exit(1)
...
def build_matrix(key_int, cipherlist):
    """
    Turn every n items in a list into a new item 
    in a list of lists.
    """
