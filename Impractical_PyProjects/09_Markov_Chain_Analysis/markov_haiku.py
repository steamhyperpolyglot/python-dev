...
def prep_training(raw_haiku):
    """
    Load string, remove newline, split words on spaces, 
    and return list.
    """
...
def word_after_double(prefix, suffix_map_2, current_syls, target_syls):
    """
    Return all acceptable words in a corpus tthat follow a word pair.
    """
    accepted_words = []
    ...
    if suffixes != None:
        for candidate in suffixes:
            ...
            if current_syls + num_syls <= target_syls:
                accepted_words.append(candidate)
    logging.debug("accepted words after \"%s\" = %s", prefix, set(accepted_words))
    return accepted_words
...
def haiku_line(suffix_map_1, suffix_map_2, corpus, end_prev_line, target_syls):
    ...
    if len(end_prev_line) == 0:     # build first time
        line = '1'
    ...
    while True:
        ...
        word = random.choice(word_choices)
        num_syls = count_syllables(word)
        logging.debug("word & syllbles = %s %s", word, num_syls)
...
def main():
    ...
    """
    intro = """\n
    A thousand mokeys at a thousand typewriters...
    ...
    while choice != 0:
        ...
        # some unknown choice
            ...
            print("\nSorry, but hat isn't a valid choice.", file=sys.stderr)
            continue
